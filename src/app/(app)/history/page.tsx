...
export default function HistoryPage() {
  ...
  const fetchAllSessions = useCallback(async () => {
    ...
    try {
      ...
      const byDate: Record<string, { total: number; perId: Record<string, number> }> = {};
      ...
      snap.docs.forEach(docSnap => {
        ...
        ids.forEach((id: string) => {
          byDate[sessionDate].perId[id] = (byDate[sessionDate].perId[id] || 0) + durationMin;
        });
      });
  ...
  const getTaskFocusTime = (task: any, date: string): number => {
    ...
    if (!dayData || !task.linkedTrackItIds || task.linkedTrackItIds.length === 0) return 0;
    let maxDuration = 0;
      ...
      <main className="mx-auto w-full max-w-3xl px-4 sm:px-6 pt-20 sm:pt-24 md:pt-28 pb-4">
        ...
        <div className="flex flex-col gap-4">
          ...
          {filteredRecords.map((record) => {
            ...
            const displayFocusTime = dayFocus?.total || record.totalFocusTime || 0;
                ...
                <div className={`flex items-center justify-between ${isExpanded ? "mb-6 sm:mb-8" : ""}`}>
                  <div className="flex items-center gap-4 sm:gap-6">
                    ...
                    <div className="flex flex-col">
                      <span className={`text-xs sm:text-sm font-bold uppercase tracking-tight ${colorClassText}`}>
                        {ratingLabel}
                      </span>
                      <span className="text-[10px] font-bold uppercase tracking-widest text-[#464555]">
                        {record.completedTasks}/{record.totalTasks} Task Clearance
                      </span>
                      {displayFocusTime > 0 && (
                        <div className="flex items-center gap-1.5 mt-1">
                          <span className="material-symbols-outlined text-[12px] text-[#4F44E2]">timer</span>
                  ...
                  <div className={`relative flex items-center justify-center shrink-0 ${isExpanded ? "h-14 w-14" : "h-10 sm:h-12 w-10 sm:w-12"}`}>
                    ...
                    <span className={`absolute font-black ${colorClassText} text-[10px] sm:text-xs`}>
                  ...
                  <div className="space-y-4 border-t border-[#464555]/15 pt-6 mt-6">
                    {record.tasks.map((task, idx) => {
                      ...
                      const timeStr =
                          ...
                          ? new Date(task.completedAt).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: false })
                          : !done ? "Miss" : "Done";
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col">
                              <span className={`text-sm font-medium ${done ? "text-[#E2E2E2]" : "text-[#464555] line-through"}`}>
                                {task.title}
                              </span>
                              {taskFocus > 0 && (
