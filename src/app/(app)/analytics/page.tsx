...
export default function AnalyticsPage() {
  ...
  const [selectedDayRecord, setSelectedDayRecord] = useState<typeof records[0] | null>(null);
  ...
  const { streak, longestStreak } = useMemo(() => {
    ...
    if (!records.find((r) => r.date === todayStr && r.completionPercentage >= 80)) {
  ...
  const { trend, weeklyCompare, categoryStats, bestDay, topHabit } = useMemo(() => {
    // Trend
    ...
    const recent = records.filter(r => r.date > fifteenDaysAgo.toLocaleDateString('en-CA'));
    const older = records.filter(r => r.date <= fifteenDaysAgo.toLocaleDateString('en-CA') && r.date > thirtyDaysAgo.toLocaleDateString('en-CA'));
    const recentAvg = recent.length ? recent.reduce((a, b) => a + b.completionPercentage, 0) / recent.length : 0;
    const olderAvg = older.length ? older.reduce((a, b) => a + b.completionPercentage, 0) / older.length : 0;
    ...
    const lastWeekAvg = lastWeekRecs.length ? lastWeekRecs.reduce((a, b) => a + b.completionPercentage, 0) / 7 : 0;
    const wDiff = Math.round(thisWeekAvg - lastWeekAvg);

    // Categories
    const catCounts: Record<string, number> = {};
    let catTotal = 0;
    records.forEach(r => r.tasks.forEach(t => {
        if (t.isCompleted) {
    ...
    const cStats = Object.entries(catCounts)
        .map(([name, count]) => ({ name, percentage: catTotal > 0 ? Math.round((count / catTotal) * 100) : 0 }))
        .sort((a, b) => b.percentage - a.percentage);

    // Best Day & Top Habit
    const bestDayRec = weeklyData.reduce((prev, curr) => (curr.completion > prev.completion ? curr : prev), weeklyData[0]);
  ...
  const handleShare = () => {
    const text = `I had a ${streak > 0 ? `${streak}-day streak` : 'great day'} on SIRA! 🔥\n` +
  ...
  const thirtyDaysAgoStr = new Date(new Date().setDate(new Date().getDate() - 30)).toLocaleDateString('en-CA');
  ...
  const handlePrevMonth = () => setCurrentMonthDate(p => new Date(p.getFullYear(), p.getMonth() - 1, 1));
  const handleNextMonth = () => setCurrentMonthDate(p => new Date(p.getFullYear(), p.getMonth() + 1, 1));
    ...
    <div className="min-h-screen bg-black text-[#E2E2E2]">
      ...
      <main className="mx-auto w-full max-w-2xl space-y-6 sm:space-y-8 px-4 sm:px-6 pt-20 sm:pt-24 md:pt-28 pb-4">
        ...
        <div className="flex items-center justify-between gap-4">
           <div>
              <h2 className="font-headline text-2xl sm:text-3xl font-black tracking-tighter text-[#E2E2E2] uppercase">Insights</h2>
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#464555]">Your performance landscape</p>
           </div>
           <button 
             onClick={handleShare}
        ...
        <section className="grid grid-cols-2 gap-2 sm:gap-4">
          ...
          <div className="flex h-24 sm:h-32 flex-col justify-between rounded-lg sm:rounded-xl border border-[rgba(70,69,85,0.2)] bg-[rgba(14,14,14,0.8)] p-3 sm:p-6 shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-all hover:bg-[#111111]">
            <div className="flex items-start justify-between gap-2">
              <span className="font-headline text-2xl sm:text-3xl font-extrabold text-[#E2E2E2]">{avgCompletion}%</span>
              <span className="material-symbols-outlined text-xs sm:text-base opacity-50 text-[#E2E2E2]">trending_up</span>
            </div>
            <div className="flex flex-col">
              <p className="text-[8px] sm:text-[10px] font-bold uppercase tracking-widest text-[#8E8D99]">Avg. Completion</p>
              <span className={`text-[7px] font-bold mt-0.5 ${trend.isUp ? 'text-[#C4C0FF]' : 'text-[#464555]'}`}>
          ...
          <div className="flex h-24 sm:h-32 flex-col justify-between rounded-lg sm:rounded-xl border border-[rgba(70,69,85,0.2)] bg-[rgba(14,14,14,0.8)] p-3 sm:p-6 shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-all hover:bg-[#111111]">
            <div className="flex items-start justify-between gap-2">
              <div>
                <span className="font-headline text-2xl sm:text-3xl font-extrabold text-[#E2E2E2]">{streak}</span>
                <span className="ml-2 text-[10px] font-bold text-[#8E8D99]">/ {longestStreak}</span>
              </div>
              <span className="material-symbols-outlined text-xs sm:text-base opacity-50 text-[#E2E2E2]" style={{ fontVariationSettings: "'FILL' 1" }}>local_fire_department</span>
            </div>
            <p className="text-[8px] sm:text-[10px] font-bold uppercase tracking-widest text-[#8E8D99]">Streak (Current / Best)</p>
          </div>

          {/* Perfect Days */}
          <div className="flex h-24 sm:h-32 flex-col justify-between rounded-lg sm:rounded-xl border border-[rgba(70,69,85,0.2)] bg-[rgba(14,14,14,0.8)] p-3 sm:p-6 shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-all hover:bg-[#111111]">
            <div className="flex items-start justify-between gap-2">
              <span className="font-headline text-2xl sm:text-3xl font-extrabold text-[#E2E2E2]">{perfectDaysCount}</span>
              <span className="material-symbols-outlined text-xs sm:text-base opacity-50 text-[#E2E2E2]" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
            </div>
            <p className="text-[8px] sm:text-[10px] font-bold uppercase tracking-widest text-[#8E8D99]">Perfect Days</p>
          </div>

          {/* Lifetime Tasks */}
          <div className="flex h-24 sm:h-32 flex-col justify-between rounded-lg sm:rounded-xl border border-[rgba(70,69,85,0.2)] bg-[rgba(14,14,14,0.8)] p-3 sm:p-6 shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-all hover:bg-[#111111]">
            <div className="flex items-start justify-between gap-2">
              <span className="font-headline text-2xl sm:text-3xl font-extrabold text-[#E2E2E2]">{totalTasksDone}</span>
              <span className="material-symbols-outlined text-xs sm:text-base opacity-50 text-[#E2E2E2]">task_alt</span>
            </div>
            <p className="text-[8px] sm:text-[10px] font-bold uppercase tracking-widest text-[#8E8D99]">Total Tasks Done</p>
          </div>
        </section>
        ...
        <section className="rounded-lg sm:rounded-xl border border-[rgba(70,69,85,0.2)] bg-[rgba(14,14,14,0.8)] p-4 sm:p-8">
          <div className="mb-4 sm:mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div className="flex flex-col">
              ...
              <span className={`text-[8px] font-bold uppercase ${weeklyCompare.isUp ? 'text-[#C4C0FF]' : 'text-[#464555]'}`}>
                {weeklyCompare.isUp ? '↑' : '↓'} {Math.abs(weeklyCompare.val)}% vs last week
              </span>
            </div>
        ...
        <section className="rounded-lg sm:rounded-xl border border-[rgba(70,69,85,0.2)] bg-linear-to-br from-[#0E0E0E] to-[#151515] p-6 sm:p-8 relative overflow-hidden">
           ...
           <h3 className="mb-6 font-headline text-xs sm:text-sm font-bold uppercase tracking-widest text-[#C4C0FF]">Weekly Report Summary</h3>
           
           <div className="grid grid-cols-2 gap-6 relative z-10">
              <div className="space-y-1">
                 <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#464555]">Best Day</span>
                 <p className="font-bold text-sm text-white">{bestDay.dayShort} • {bestDay.completion}%</p>
              </div>
              <div className="space-y-1">
                 <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#464555]">Top Habit</span>
                 <p className="font-bold text-sm text-white uppercase">{topHabit}</p>
              </div>
              <div className="space-y-1">
                 <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#464555]">Weekly Delta</span>
                 <p className={`font-bold text-sm ${weeklyCompare.isUp ? 'text-[#E2E2E2]' : 'text-[#8E8D99]'}`}>
                   {weeklyCompare.isUp ? '+' : ''}{weeklyCompare.val}% <span className="text-[10px] text-[#464555] ml-1">vs last week</span>
                 </p>
              </div>
              <div className="space-y-1">
                 <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#464555]">Performance</span>
        ...
        <section className="rounded-lg sm:rounded-xl border border-[rgba(70,69,85,0.2)] bg-[rgba(14,14,14,0.8)] p-4 sm:p-8">
           <h3 className="mb-6 font-headline text-xs sm:text-sm font-bold uppercase tracking-widest">Category Allocation</h3>
           <div className="space-y-4">
              {categoryStats.length === 0 ? (
                <p className="text-xs text-[#464555] text-center">No task data available.</p>
                   ...
                   <div className="h-1.5 w-full rounded-full bg-[#111111] overflow-hidden">
                      <div 
                        ...
                        style={{ 
                            ...
                            backgroundColor: i === 0 ? '#E2E2E2' : i === 1 ? '#C4C0FF' : i === 2 ? '#4F44E2' : i === 3 ? '#464555' : '#1A1A1A',
                        }} 
                      />
                   </div>
                </div>
            ...
            {calendarDays.map((dateObj, i) => {
              ...
              if (record) {
                if (record.rating === "perfect")
                  colorClass = "bg-[#7cf6ec] shadow-[0_0_10px_rgba(124,246,236,0.3)]";
                else if (record.rating === "great")
                  colorClass = "bg-[#C4C0FF]";
          ...
          {selectedDayRecord && (
            <div className="mt-8 rounded-lg border border-[rgba(70,69,85,0.2)] bg-[#111111]/80 p-4 animate-fade-in shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                <div className="flex justify-between items-center mb-4">
                    <div className="flex flex-col">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[#8E8D99]">Selected Audit</span>
                        <span className="text-sm font-bold text-[#E2E2E2]">{new Date(selectedDayRecord.date).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                    </div>
                    <button onClick={() => setSelectedDayRecord(null)} className="text-[#464555] hover:text-[#E2E2E2] transition-colors rounded-full p-1 bg-black">
                        <span className="material-symbols-outlined text-lg">close</span>
                    </button>
                </div>
                <div className="space-y-2">
                    {selectedDayRecord.tasks.length === 0 ? (
                        <p className="text-center text-xs text-[#464555] py-2">No tasks recorded.</p>
                    ) : selectedDayRecord.tasks.map((t, idx) => (
                        <div key={idx} className="flex items-center gap-3 py-1">
                            <span className={`material-symbols-outlined text-sm ${t.isCompleted ? 'text-[#E2E2E2]' : 'text-[#464555]'}`}>
                                {t.isCompleted ? 'check_circle' : 'circle'}
                            </span>
                            <span className={`text-xs ${t.isCompleted ? 'text-[#E2E2E2]' : 'text-[#464555] line-through'}`}>{t.title}</span>
                        </div>
                    ))}
                </div>
            </div>
